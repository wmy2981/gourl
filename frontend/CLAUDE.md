...
- **Connection card** (`ConnectionCard`, Settings.tsx) always renders, independent of the config loading state; probes `api.authStatus()` every 10s + on app foreground: `authenticated: true` → connected, false (or a 401) → unauthorized, network error → unreachable; a legacy server without the authenticated field falls back to the config probe's 401 judgment
