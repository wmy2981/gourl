...
- Motion: CSS tokens (`--animate-*` in index.css) for pages/dialogs, `motion` (framer-motion) for toasts (stacked pile, newest in front, rear cards peek 12px; cards are right-aligned via `items-end` so varying widths share a right edge). **No global `prefers-reduced-motion` kill-switch — removed on purpose** (the owner's OS has it enabled and wants the animations)
...
- Batch create (`BatchCreateDialog` + `lib/batch.ts`): strict lines `[code](date)url`; validation runs **on editor blur or Create click — never on every keystroke** (`CodeEditor.onBlur`, jsdom needs the focus-timer settle in tests); keeps failed lines editable for the retry button
- Import (`ImportDialog`): JSON accepts both the legacy bare array and the export dump (`{meta, items}` — unwrap `.items`); CSV file import skips the exporter's `#`-comment metadata lines before parsing. The ExportDialog offers CSV / JSON / Markdown — all three downloads go through `withBusy` + `saveDownload`
- Editor token colors (`--tok-*` in index.css, applied via `.tok-*` classes) follow the **VSCode palettes**: One Dark in dark mode (orange keys/code, green strings/dates, blue urls/numbers), deepened Light+ equivalents in light mode. Batch-create lines use `lib/batchHighlight.ts` (custom tags), import JSON uses `lib/jsonHighlight.ts` (lezer tags mapped onto the same classes)
- The CodeMirror shell styles are scoped to `.cm-editor.batch-editor`: opaque gutter (`#fafafa` / `#171719` — it is position: sticky and translucent backgrounds let scrolled content bleed through), `overflow: hidden` to keep line washes inside the rounded frame, and `outline: none` on `.cm-focused` (the default dotted outline clashes with the amber focus ring)
- jsdom lacks DOM APIs CodeMirror needs — the shared stubs live in `src/test/setup.ts` (ResizeObserver + Range client rects); CodeMirror dispatches focus changes on a ~10ms timer, so blur tests must settle between focus and blur
- Log page: SSE via `api.logStream`, auto-reconnects, history oldest-first; attrs are hidden below `sm` so messages never collapse on phones; the .log export runs through `saveDownload` (`exportFilename('logs', 'log')` → Downloads/gourl/ in the app, same as json/csv)
...
- Downloads via `lib/download.ts` (`saveDownload`, used by QRDialog + ExportDialog + the log export): `<a download>` on web; in the app, system **Downloads/gourl/** via `@capgo/capacitor-file-sharer` with the path toasted — never `@capacitor/filesystem` (its v8 enum lost the Downloads directory). Export filenames come from `exportFilename(kind, ext)`: `gourl-links|logs-<local yyyy-mm-dd-hh-mm-ss>.{csv,json,log}` (local time, matching the backend CSV header)
