...
- **Stable ids & edit snapshots** — every link carries a stable autoincrement id; each edit appends an immutable snapshot to the backups table (`b-1, b-2, …`), and the db-export script dumps links (soft-deleted included), tokens, daily clicks and backups as JSON arrays
