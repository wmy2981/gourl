...
- Logging: `log/slog` via `internal/logx` — 4 levels (debug/info/warning/error). **The level is a business config field** (`log_level`, settings page, hot-applied via `logx.SetLevel`); `LOG_DIR` defaults to `./data/log` (the mounted `/app/data/log` in the container) and mirrors logs to a rotating file on the volume; `LOG_FORMAT` stays env-only. English messages only, no i18n. **Request access logs sit at debug; info carries business events** (login, link create/update/delete, batch ops, config changes, UA/IP blocks, token management — always with `actor` = session|token). `logx` fans every record to subscribers (log page SSE) and reads history back from the LOG_DIR file
...
- **Admin password** is a bcrypt hash in `config.yaml` (`password_hash`, hidden from the JSON contract) set by the setup flow (`POST /api/v1/auth/setup` + `/admin/setup` page) or migrated once from a legacy `ADMIN_PASSWORD` env var. `PUT /api/v1/config` must carry the existing hash over or it gets wiped. With no hash the server runs in setup mode: management endpoints refuse with 403 `setup_required` (bearer tokens still work; `/admin/setup`, `/api/v1/auth/status`, `/api/v1/health` and short-link redirects stay open). `SESSION_SECRET` signs session tokens; when unset the server generates a strong ephemeral one at startup (in-memory only — sessions do not survive a restart)
- **Rate limiting**: failed logins lock the client IP (`login_rate_max_attempts`/`login_rate_lock_seconds`, default 10/300, 429 + `rate_limited`); short-link redirects share a per-second token bucket (`link_rate_per_second`, default 100, over-limit = bare 429, no redirect, no click). Both 0 = disabled, both hot-reloadable
